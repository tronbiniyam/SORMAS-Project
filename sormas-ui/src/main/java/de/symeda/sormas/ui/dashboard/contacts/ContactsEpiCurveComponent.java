...
public class ContactsEpiCurveComponent extends AbstractEpiCurveComponent {
	...
	public void clearAndFillEpiCurveChart() {
		...
		hcjs.append(
			"var options = {" + "chart:{ " + " type: 'column', " + " backgroundColor: 'transparent' " + "}," + "credits:{ enabled: false },"
				+ "exporting:{ " + " enabled: true," + " buttons:{ contextButton:{ theme:{ fill: 'transparent' } } }" + "}," + "title:{ text: '' },");
		...
		hcjs.append(
			"yAxis: { min: 0, title: { text: '" + I18nProperties.getCaption(Captions.dashboardNumberOfContacts)
				+ "' }, allowDecimals: false, softMax: 10, " + "stackLabels: { enabled: true, "
