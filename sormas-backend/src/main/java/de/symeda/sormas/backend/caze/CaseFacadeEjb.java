...
public class CaseFacadeEjb implements CaseFacade {
	...
	public List<CaseExportDto> getExportList(
		...
		cq.multiselect(
			...
			personAddressJoin.get(Location.ID),
			epiDataJoin.get(EpiData.ID),
			symptomsJoin.get(Symptoms.ID),
			hospitalizationJoin.get(Hospitalization.ID),
			districtJoin.get(District.ID),
			healthConditionsJoin.get(HealthConditions.ID),
			caseRoot.get(Case.UUID),
			caseRoot.get(Case.EPID_NUMBER),
			caseRoot.get(Case.DISEASE),
			caseRoot.get(Case.DISEASE_DETAILS),
			personJoin.get(Person.FIRST_NAME),
			personJoin.get(Person.LAST_NAME),
			personJoin.get(Person.SEX),
			caseRoot.get(Case.PREGNANT),
			personJoin.get(Person.APPROXIMATE_AGE),
			personJoin.get(Person.APPROXIMATE_AGE_TYPE),
			personJoin.get(Person.BIRTHDATE_DD),
			personJoin.get(Person.BIRTHDATE_MM),
			personJoin.get(Person.BIRTHDATE_YYYY),
			caseRoot.get(Case.REPORT_DATE),
			regionJoin.get(Region.NAME),
			districtJoin.get(District.NAME),
			communityJoin.get(Community.NAME),
			facilityJoin.get(Facility.NAME),
			facilityJoin.get(Facility.UUID),
			caseRoot.get(Case.HEALTH_FACILITY_DETAILS),
			pointOfEntryJoin.get(PointOfEntry.NAME),
			pointOfEntryJoin.get(PointOfEntry.UUID),
			caseRoot.get(Case.POINT_OF_ENTRY_DETAILS),
			caseRoot.get(Case.CASE_CLASSIFICATION),
			caseRoot.get(Case.INVESTIGATION_STATUS),
			caseRoot.get(Case.OUTCOME),
			// quarantine
			...
			hospitalizationJoin.get(Hospitalization.LEFT_AGAINST_ADVICE),
			personJoin.get(Person.PRESENT_CONDITION),
			personJoin.get(Person.DEATH_DATE),
			personJoin.get(Person.BURIAL_DATE),
			personJoin.get(Person.BURIAL_CONDUCTOR),
			personJoin.get(Person.BURIAL_PLACE_DESCRIPTION),
			// address
			personAddressRegionJoin.get(Region.NAME),
			personAddressDistrictJoin.get(District.NAME),
			personAddressJoin.get(Location.CITY),
			personAddressJoin.get(Location.ADDRESS),
			personAddressJoin.get(Location.POSTAL_CODE),
			// phone
			personJoin.get(Person.PHONE),
			personJoin.get(Person.PHONE_OWNER),
			personJoin.get(Person.EDUCATION_TYPE),
			personJoin.get(Person.EDUCATION_DETAILS),
			personJoin.get(Person.OCCUPATION_TYPE),
			personJoin.get(Person.OCCUPATION_DETAILS),
			occupationFacilityJoin.get(Facility.NAME),
			occupationFacilityJoin.get(Facility.UUID),
			personJoin.get(Person.OCCUPATION_FACILITY_DETAILS),
			epiDataJoin.get(EpiData.TRAVELED),
			epiDataJoin.get(EpiData.BURIAL_ATTENDED),
			epiDataJoin.get(EpiData.DIRECT_CONTACT_CONFIRMED_CASE),
			epiDataJoin.get(EpiData.DIRECT_CONTACT_PROBABLE_CASE),
			epiDataJoin.get(EpiData.RODENTS),
			caseRoot.get(Case.VACCINATION),
			caseRoot.get(Case.VACCINATION_DOSES),
			caseRoot.get(Case.VACCINATION_DATE),
			caseRoot.get(Case.VACCINATION_INFO_SOURCE),
			caseRoot.get(Case.POSTPARTUM),
			caseRoot.get(Case.TRIMESTER));
		...
		if (configFacade.isFeatureAutomaticCaseClassification()) {
			if (newCase.getCaseClassification() != CaseClassification.NO_CASE) {
				...
				List<PathogenTestDto> sampleTests =
					pathogenTestService.getAllByCase(newCase).stream().map(s -> sampleTestFacade.toDto(s)).collect(Collectors.toList());
	...
	void archiveAllArchivableCases(int daysAfterCaseGetsArchived, LocalDate referenceDate) {
		...
		cq.where(cb.equal(from.get(Case.ARCHIVED), false), cb.not(caseService.createChangeDateFilter(cb, from, notChangedTimestamp)));
