...
public class SampleFacadeEjb implements SampleFacade {
	...
	public List<SampleDto> getAllActiveSamplesAfter(Date date) {
		...
		return sampleService.getAllActiveSamplesAfter(date, user).stream().map(e -> toDto(e)).collect(Collectors.toList());
	}

	@Override
	public List<SampleDto> getByUuids(List<String> uuids) {
		return sampleService.getByUuids(uuids).stream().map(c -> toDto(c)).collect(Collectors.toList());
	}

	@Override
	public List<SampleDto> getByCaseUuids(List<String> caseUuids) {
		return sampleService.getByCaseUuids(caseUuids).stream().map(c -> toDto(c)).collect(Collectors.toList());
	}
	...
	public List<SampleIndexDto> getIndexList(SampleCriteria sampleCriteria, Integer first, Integer max, List<SortProperty> sortProperties) {
		...
		final Expression diseaseSelect =
			qc.addExpression(DISEASE, cb.selectCase().when(cb.isNotNull(caze), caze.get(Case.DISEASE)).otherwise(contact.get(Contact.DISEASE)));
		final Expression diseaseDetailsSelect = qc.addExpression(
			DISEASE_DETAILS,
			cb.selectCase().when(cb.isNotNull(caze), caze.get(Case.DISEASE_DETAILS)).otherwise(contact.get(Contact.DISEASE_DETAILS)));

		final Expression regionSelect = qc.addExpression(
			REGION,
			cb.selectCase()
				.when(cb.isNotNull(caseRegion), caseRegion.get(Region.UUID))
				.otherwise(
					cb.selectCase().when(cb.isNotNull(contactRegion), contactRegion.get(Region.UUID)).otherwise(contactCaseRegion.get(Region.UUID))));

		final Expression districtSelect = qc.addExpression(
			DISTRICT,
			cb.selectCase()
		...
		final Expression districtNameSelect = qc.addExpression(
			DISTRICT_NAME,
			cb.selectCase()
				...
				.otherwise(
					cb.selectCase()
						...
						.otherwise(contactCaseDistrict.get(District.NAME))));
		...
		cq.multiselect(
			...
			lab.get(Facility.UUID),
			lab.get(Facility.NAME),
			referredSample.get(Sample.UUID),
			caze.get(Case.UUID),
